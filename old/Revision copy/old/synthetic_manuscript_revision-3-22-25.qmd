...
fluency_data_original <- rio::import(file = here::here("Data", "03_Fluency", "data_Dyslexia Stutter Master.csv")) |>
  ...
  dplyr::select(Group, Nonwordrepetition)
...
# Analysis Function ----
# Examine correlation between education (years) and reading t-scores
language_analyze_data2 <- function(data) {
  
  #Friedman test on overall language scores at 5 time points
  model <- friedman.test(y=data$scores, groups=data$time, blocks=data$item)
  
  list(
    p_value = model$p.value, # Extracting p-values
    effect_size = model$statistic, # Extracting effect size
    conf_int_lower = NA,
    conf_int_upper = NA
  )
}
...
# Reproduce original results ----
language_results_original2 <- language_analyze_data2(language_data_original2) |>
  as.data.frame(row.names = NULL) |>
  mutate(
    dataset = "original",
    sig = case_when(
...
# Create synthetic data ----
language_data_synthetic2 <- syn(language_data_original2, m = 100, seed = 2024)

### STUCK HERE - Need to loop through each synthetic dataset and run the friedman's test but keep getting an error

# Perform analysis on each synthetic dataset
language_results_synthetic2 <- language_data_synthetic2$syn |>
  purrr::map_df(~ language_analyze_data2(.x), .id = "dataset") |>
  dplyr::mutate(
    sig = case_when(
      p_value < language_alpha ~ "sig",
      p_value >= language_alpha ~ "non-sig"
    ...
    categorization = case_when(abs(effect_size) >= 0.5 ~ "Large",
                               ...
                               TRUE ~ "Negligible"),
    # Calculate the difference between the original p-value/effect size and synthetic p-value/effect sizes.
    diff_pvalue = abs(p_value - language_results_original2$p_value),
    diff_effectSize = abs(effect_size - language_results_original2$effect_size)
  ) |> 
  add_column(domain = "Receptive and expressive language")
