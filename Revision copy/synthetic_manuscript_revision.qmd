...
# Analysis Function ----
# Examine correlation between education (years) and reading t-scores
language2_analyze_data <- function(data) {
  ...
  median_qi <- es %>% # only need one variable
  tidybayes::median_qi(effect_size)
  
  list(
    effect_size = median_qi$effect_size, # Extracting effect size
    # CI
    conf_int_lower = median_qi$.lower,
    conf_int_upper = median_qi$.upper
  )
}

# Reproduce original results ----
language_results_original <- language2_analyze_data(df_coded2) |>
  as.data.frame(row.names = NULL) |>
  mutate(
    dataset = "original"
  )
...
# Create synthetic data ----
language2_data_synthetic <- syn(df_coded2, # method = "ctree",
                                m = 100, seed = 2024)

# Perform analysis on each synthetic dataset
language2_results_synthetic <- language2_data_synthetic$syn |>
  purrr::map_df(~ language2_analyze_data(.x), .id = "dataset") |>
  add_column(domain = "Receptive and expressive language")
