...
```{r recep_language_2, echo = FALSE}
library(brms)

# Import original data ----
robinaugh_df <- read_csv(here::here("Data", "07_Language", "Robinaugh", "SDTBI001_Data.csv")) %>%
  janitor::clean_names() %>%
...
# Establish p_value for study
## no p-value - bayesian model

# Data wrangling
df_coded2 <- robinaugh_df %>%
  filter(session < 13) %>%
  group_by(id) %>%
  ...
  select(-condition) |> 
  select(c(id, session, slope_change, response, set, phase))

# Analysis Function ----
# fit bayesian interrupted time series model
...
posterior_samples(model_name)[,1:4] %>%
 ...
 #  kableExtra::kable(digits = 2, caption = "M.1b model summary")
...
es <- tidybayes::add_epred_draws(m.1b, newdata = pred_d |> filter(set != 7), 
                      pred = 'value', seed = 42) %>% 
  ungroup() %>% #
...
# Examine correlation between education (years) and reading t-scores
robinaugh_analyze_data <- function(data) {
  
  model <- brm(response ~ 0 + Intercept + session + phase + slope_change + 
             (session + phase + slope_change | set/id), # 
           family = bernoulli(),  #binomial in lme4
           data = data, # both trained and untrained sets
           iter = 4000, # number of total samples per chain
           warmup = 1000, # number of samples tossed out per chain
           inits = 'random', # each chain takes random starting values
           chains = 4, # 4 total chains
  ...
  # Obtain Effect size estimate of trained words
  pred_d <- model$data %>%
  select(-response, -Intercept) %>% 
  group_by(id) %>% # for each word
  mutate(num_baselines = ifelse(set != 7, sum(phase == 0), 4)) %>% 
  ...
  es <- tidybayes::add_epred_draws(
    model,
    newdata = pred_d |> filter(set != 7),
    pred = 'value',
    seed = 42) %>%
    ungroup() %>% #
    mutate(time_point = ifelse(slope_change == 0, 'entry', 'exit')) %>% #
  ...
  list(
    p_value = NA, # Extracting p-values
    effect_size = es$effect_size, # Extracting effect size
    # CI
    .lower = es$.lower,
    .upper = es$.upper
  )
...
# Reproduce original results ----
robinaugh_results_original <- robinaugh_analyze_data(df_coded2) |>
  as.data.frame(row.names = NULL) |>
  mutate(
    dataset = "original",
    sig = NA,
...
# Create synthetic data ----
robinaugh_data_synthetic <- syn(df_coded2, # method = "ctree",
                                m = 3, seed = 2024)

# Perform analysis on each synthetic dataset
robinaugh_results_synthetic <- robinaugh_data_synthetic$syn |>
  purrr::map_df(~ robinaugh_analyze_data(.x), .id = "dataset") |>
  dplyr::mutate(
    sig = NA,
    categorization = NA,
  ) |> 
...
# Compile results ----
robinaugh__results_summary <- data_frame(
  Domain = "Language",
  Study = "Robinaugh et al. (2024)",
  
  # Pulling sample size, p value, and effect sizes from the original results
  N = 1,
  p_value = NA,
  effect_size_measure = "Median of posterior distribution",
  effect_size = es$effect_size,
  
  # Creating the % Agreement for p values between the original and the synthetic datasets
  synAgreement_pvalue = NA,
  
  # Creating the % Agreement for effect sizes between the original and the synthetic datasets
  synAgreement_effectSize = robinaugh_results_synthetic |>
    count(categorization) |>
    dplyr::mutate(percent = n / nrow(language_results_synthetic) * 100,
                  percent = sprintf("%0.0f%%", percent)) |> # Formatting to be a %
    dplyr::filter(categorization == language_results_original$categorization) |>
...
# Saving the data and results for easy importing later.
base::saveRDS(object = language,
              file = here::here("Data","07_Language", "Robinaugh", "analysisData.RDS"))
```
