...
# Load manuscript version
manuscript_version <- params$manuscript_version
...
fileout <- here("manuscript", manuscript_version, "tables-figures", "table1.docx") # uncomment to write in your working directory
...
doc <- read_docx(here("manuscript", "templates-data", "template.docx"))
doc <- body_add_flextable(doc, value = t2)
fileout <- here("manuscript", manuscript_version, "tables-figures", "table2.docx") # uncomment to write in your working directory
print(doc, target = fileout)
```
...
fileout <- here("manuscript", manuscript_version, "tables-figures", "table3.docx")
print(doc, target = fileout)
```
...
swallowing <- readRDS(here::here("Manuscript", "0_initial", "result_data","01_Swallowing","analysisData.RDS"))

swallowing_data_synthetic_first <- swallowing$data_synthetic[[1]]

# General Utility ----
swallowing_data_original <- swallowing$data_original |> 
  dplyr::mutate(dataset_type = "Original")
...
# Load previous results
articulation <- readRDS(here::here("Manuscript", "1_rev1", "result_data","02_Articulation","analysisData.RDS"))


articulation_data_synthetic_first <- articulation$data_synthetic$syn[[1]]
...
@thompson_etal23 examined the relationship between vowel space area and speech intelligibility among 40 speakers with dysarthria of varying etiologies (e.g., Parkinson's disease, amyotrophic lateral sclerosis, Huntington's disease, and ataxia). A linear regression model revealed a statistically significant relationship between vowel space area and intelligibility (*p* \< .001) with a Cohen's *f* of 0.59, corresponding to a conventionally "large" effect size (Table 3).

Compared to original data, the synthetic data demonstrated a similar distribution (Figure 2B). General utility was high for both variables of vowel space area (*S_pMSE* = `r round(articulation_s_pMSE_value$S_pMSE[1], digits = 2)`) and intelligibility (*S_pMSE* = `r round(articulation_s_pMSE_value$S_pMSE[2], digits = 2)`). Specific utility was high as the statistical model with the synthetic data maintained the direction of statistical significance (*p* = `r fmt_p_value(articulation$results_synthetic[1,]$p_value)`) and effect size magnitude (*f* = `r round(articulation$results_synthetic[1,]$effect_size, digits = 2)`).
```{r echo = FALSE}
articulation <- readRDS(here::here("Manuscript", "0_initial", "result_data","02_Articulation","analysisData.RDS"))
```

Findings from the 100 generated synthetic datasets indicate that `r articulation$results_summary$synAgreement_pvalue` of datasets demonstrated the same inferential result (i.e., a statistically significant *p*-value) and `r articulation$results_summary$synAgreement_effectSize` maintained their effect size category.
...
# Load previous results
fluency <- base::readRDS(file = here::here("Manuscript", "1_rev1", "result_data","03_Fluency","analysisData.RDS"))

fluency_data_synthetic_first <- fluency$data_synthetic$syn[[1]]
...
@elsherif_etal21 compared non-word repetitions between 80 neurotypical adults and 34 adults who stutter. An independent samples t-test demonstrated a statistically significant difference in non-word repetitions between these groups (*p* \< .001) with a large effect size (Δ = 1.26).

Compared to original data, the synthetic data demonstrated a similar distribution (Figure 2C). General utility was high for the outcome of non-word repetitions (*S_pMSE* = `r round(fluency_s_pMSE_value$S_pMSE[3], digits = 2)`) and the statistical model with the synthetic data maintained the direction of statistical significance (*p* = `r fmt_p_value(fluency$results_synthetic[1,]$diff_pvalue)`) and effect size magnitude (*Δ* = `r abs(round(fluency$results_synthetic[1,]$effect_size, digits = 2))`).

Findings from the 100 generated synthetic datasets indicate that `r fluency$results_summary$synAgreement_pvalue` of datasets demonstrated the same inferential result (i.e., a statistically significant *p*-value) and maintained a ‘large’ effect size categorization.

### Voice and Resonance

```{r echo = FALSE}
# Load previous results
voice <- base::readRDS(file = here::here("Manuscript", "1_rev1", "result_data","04_Voice_Resonance","analysisData.RDS"))

voice_data_synthetic_first <- voice$data_synthetic$syn[[1]]
# General Utility ----

# Combine the data frames
voice_combined_data <- voice_data_synthetic_first |> 
  mutate(dataset_type = "Synthetic") |> 
  rbind(voice$data_original)
...
Battal et al. (2019) compared auditory localization abilities between 17 congenitally blind and 17 sighted individuals. A linear mixed effects model indicated that congenitally blind individuals had enhanced spatial hearing abilities (*OR* = 1.56, *p* = .016). Compared to original data, the synthetic data showed similar distributions for auditory localization in both sighted and congenitally blind individuals, as well as similar auditory localization at the subject-level (Figure 2E). General utility was high for subject (*S_pMSE* = `r round(hearing_s_pMSE_value$S_pMSE[3], digits = 2)`) and group (*S_pMSE* = `r round(hearing_s_pMSE_value$S_pMSE[2], digits = 2)`) variables. Specific utility was high as the statistical model with the synthetic data maintained the direction of statistical significance (*p* = `r fmt_p_value(hearing$results_synthetic[1,]$p_value)`) and effect size magnitude (*OR* = `r round(hearing$results_synthetic[1,]$effect_size, digits = 2)`). The random effect estimates were stable between the original (mean = `r round(hearing$results_original$random_mean, digits = 3)`, 95% CI: `r round(hearing$results_original$random_conf_int_lower, digits = 3)`, `r round(hearing$results_original$random_conf_int_upper, digits = 3)`) and synthetic (mean = `r round(hearing$results_synthetic$random_mean[1], digits = 3)`, 95% CI: `r round(hearing$results_synthetic$random_conf_int_lower[1], digits = 3)`, `r round(hearing$results_synthetic$random_conf_int_upper[1], digits = 3)`) datasets.

Findings from the 100 generated synthetic datasets indicate that `r hearing$results_summary$synAgreement_pvalue` of datasets demonstrated the same inferential result (i.e., a statistically significant p-value) and `r hearing$results_summary$synAgreement_effectSize` maintained their effect size categorization.

### Communication Modalities

```{r echo = FALSE}
# Load previous results
aac <- base::readRDS(file = here::here("Manuscript", "1_rev1", "result_data","06_AAC","analysisData.RDS"))

# General Utility ----
aac_data_synthetic_first <- aac$data_synthetic$syn[[1]] |> 
  as.data.frame()

aac_data_original2 <- aac$data_original |> 
  mutate(dataset_type = "Original") |> 
  as.data.frame()
...
King et al. (2022) collected survey responses from speech-language pathologists to assess the impact of the COVID-19 pandemic on service provision for emergent bilinguals who use augmentative and alternative communication. Results indicated that speech-language pathologists reporting that a lack of or limited access to internet increased during the initial phase of the pandemic (Cohen's 𝜔 = 34.60, *p* \< .001). Compared to original data, the synthetic data showed similar frequencies of responses for the barrier of 'lack of/limited internet' (Figure 2F). General utility was high for assessment type (*S_pMSE* = `r round(aac_s_pMSE_value$S_pMSE[1], digits = 2)`) and time point (*S_pMSE* = `r round(hearing_s_pMSE_value$S_pMSE[2], digits = 2)`) variables. The statistical model with the synthetic data maintained the direction of statistical significance (*p* `r fmt_p_value(aac$results_synthetic[1,]$p_value)`) and effect size magnitude (Cohen’s 𝜔 = `r round(aac$results_synthetic[1,]$effect_size, digits = 2)`), indicating high specific utility.

Findings from the 100 generated synthetic datasets indicate that `r aac$results_summary$synAgreement_pvalue` of datasets demonstrated the same inferential result (i.e., a statistically significant p-value) and maintained their effect size categorization.
...
# KEARNEY
# Load previous results
language <- base::readRDS(file = here::here("Manuscript", "1_rev1", "result_data","07_Language","Kearney", "analysisData.RDS"))

# General Utility ----
language_data_synthetic_first <- language$data_synthetic$syn[[1]] |> 
  as.data.frame()

language_data_original2 <- language$data_original |> 
  mutate(dataset_type = "Original") |> 
  as.data.frame()
...
Kearney et al. (2023) examined the relationship between years of education and reading performance among 36 individuals following left-hemisphere tumor resection. Results indicated a large relationship between these variables (*r* = 0.59, *p* \< .001). Compared to original data, the synthetic data showed maintained a similar visual relationship between years of education and reading scores (Figure 3A). General utility was high for both years of education (*S_pMSE* = `r round(language_s_pMSE_value$S_pMSE[1], digits = 2)`) and reading scores (*S_pMSE* = `r round(language_s_pMSE_value$S_pMSE[2], digits = 2)`) variables. The statistical model with the synthetic data maintained the direction of statistical significance (*p* = `r fmt_p_value(language$results_synthetic[1,]$p_value)`) and effect size magnitude (*r* = `r round(language$results_synthetic[1,]$effect_size, digits = 2)`), indicating high specific utility. Findings from the 100 generated synthetic datasets indicate that `r language$results_summary$synAgreement_pvalue` of datasets demonstrated the same inferential result (i.e., a statistically significant p-value) and `r language$results_summary$synAgreement_effectSize` of synthetic datasets maintained their effect size category.
...
m.1b <- brm(response ~ 0 + Intercept + session + phase + slope_change + 
           ...
           file = here::here("Manuscript", manuscript_version, "result_data", "07_Language", "Robinaugh", "new_approach"),
...
synthetic_model <- brm(response ~ 0 + Intercept + session + phase + slope_change + 
           ...
           seed = 42,
           refresh = 0, #for the markdown document 
           file = here::here("Manuscript", manuscript_version, "result_data", "07_Language", "Robinaugh", "synthetic_model"),
           file_refit = "on_change"
)
...
cognition <- base::readRDS(here::here("Manuscript", "1_rev1", "result_data", "08_Cognition", "analysisData.RDS"))


# General Utility ----
cognition_data_synthetic_first <- cognition$data_synthetic$syn[[1]] |> 
  as.data.frame()
...
# Data viz
cognition_p1 <- cognition_combined_data |>
  dplyr::mutate(
    Correct = as.numeric(as.character(Correct)),
    Group_NC = case_when(
      Group_NC == "BasicFace" ~ "Basic Face",
  ...
  labs(x = "", y = "Proportion Correct", fill = "Group NC") +
  cowplot::theme_cowplot() +
  theme(
...
```{r echo = FALSE}
social <- base::readRDS(here::here("Manuscript", "1_rev1", "result_data", "09_Social_Communication", "analysisData.RDS"))

# General Utility ----
social_data_synthetic_first <- social$data_synthetic$syn[[1]] |> 
  as.data.frame()

social_data_original <- social$data_original |> 
  mutate(dataset_type = "Original") |> 
  as.data.frame()
...
Chanchaochai & Schwarz et al. (2023) compared non-verbal IQ between individuals with autism spectrum disorder and neurotypical peers. An analysis of variance indicated that neurotypical individuals demonstrated higher non-verbal IQ (*d* = -0.85, *p* \< .001). Compared to original data, the synthetic data showed similar distributions of non-verbal IQ for both groups (Figure 3D). General utility was high for both group (*S_pMSE* = `r round(social_s_pMSE_value$S_pMSE[2], digits = 2)`) and non-verbal IQ (*S_pMSE* = `r round(social_s_pMSE_value$S_pMSE[1], digits = 2)`) variables. The statistical model with the synthetic data maintained the direction of statistical significance (*p* = `r fmt_p_value(social$results_synthetic[1,]$p_value)`); however, the effect size magnitude (*d* = `r round(social$results_synthetic[1,]$effect_size, digits = 2)`) was lower, indicating a low level of specific utility.

Findings from the 100 generated synthetic datasets indicate that `r social$results_summary$synAgreement_pvalue` of datasets demonstrated the same inferential result (i.e., a statistically significant p-value) and `r social$results_summary$synAgreement_effectSize` of synthetic datasets maintained their effect size category.
...
swallowing_combined_data <- swallowing_data_synthetic_first |> 
  mutate(dataset_type = "Synthetic") |> 
  rbind(swallowing$data_original)
...
# save figure
ggsave(
  filename = here::here("Manuscript", manuscript_version, "tables-figures", "figure_2.png"),
    combined_plots2,
    height = 15,
    width = 13,
    dpi = 300,
...
# save figure
ggsave(
  filename = here::here("Manuscript", manuscript_version, "tables-figures", "figure_3.png"),
    combined_plots_2,
    height = 10,
    width = 10,
    dpi = 300,
...
# save figure
ggsave(
  filename = here::here("Manuscript", manuscript_version, "tables-figures", "supplemental_figure_A.png"),
    pval_combined,
    height = 10,
    width = 10,
    dpi = 300,
...
# save figure
ggsave(
  filename = here::here("Manuscript", manuscript_version, "tables-figures", "supplemental_figure_B.png"),
    es_combined,
    height = 10,
    width = 10,
    dpi = 300,
