...
synthetic_results_language2 <- synthetic_results %>%
  mutate(
...
results_summary <- tibble(
  ...
  mean_synthetic_effect_size = mean(synthetic_results_language2$synthetic_effect_size),
  mean_diff_effect_size = mean(synthetic_results_language2$diff_effect_size),
  sd_diff_effect_size = sd(synthetic_results_language2$diff_effect_size),
  mean_diff_CI_lower = mean(synthetic_results_language2$diff_CI_lower),
  mean_diff_CI_upper = mean(synthetic_results_language2$diff_CI_upper)
)
...
# Data Viz
language2_data_synthetic_first <- syn_data$syn[[1]] |> 
  as.data.frame()

language2_data_original <- df_coded2 |> 
  mutate(dataset_type = "Original") |> 
  as.data.frame()

# Combine the data frames
robinaugh_combined_data <- language2_data_synthetic_first |> 
  mutate(dataset_type = "Synthetic") |> 
  rbind(language2_data_original)
...
language_es_viz2 <- 
  synthetic_results_language2 |> 
  ggplot(aes(x = diff_effect_size)) +
  # geom_rect(aes(xmin=0.50, xmax=Inf, ymin=0, ymax=Inf),
  #           color = "#e5f5f9", fill = "#e5f5f9") +
  geom_histogram(bins = 50, color = "#66c2a4", fill = "#66c2a4") +
  cowplot::theme_cowplot() +
  ...
  theme(legend.position = "top",
        ...
        axis.title.x = element_text(margin = margin(t = 4)),
        axis.title.y = element_text(margin = margin(r = 2))) +
  labs(x = "Difference in Model Estimate",
       y = "Count") +
  ggtitle("F: Receptive and expressive language\n(Robinaugh et al., 2024)",
          subtitle = "Mean difference of 34 words (SD = 8.98)")
  # geom_vline(xintercept = 0.59, color = "black", 
  #            size = 0.75, linetype = "dashed", alpha = 0.75)
...
aac_es_viz <- 
  ...
  labs(x = "Cohen's 𝜔",
       y = "Count") +
  ggtitle("G: Communication modalities",
          subtitle = "Mean difference of 7.24 (SD = 5.08)") +
  geom_vline(xintercept = 34.6, color = "black", 
             size = 0.75, linetype = "dashed", alpha = 0.75)
...
social_es_viz <- 
  ...
  labs(x = "√(3/𝜋) x odds ratio",
       y = "Count") +
  ggtitle("I: Social aspects of communication",
          subtitle = "Mean difference of 0.21 (SD = 0.20)") +
  geom_vline(xintercept = -0.85, color = "black", 
             size = 0.75, linetype = "dashed", alpha = 0.75)
...
# save figure
ggsave(
  filename = here::here("Manuscript", manuscript_version, "tables-figures", "supplemental_figure_B-2.png"),
    es_combined,
    height = 10,
    width = 15,
    dpi = 300,
...
Appendix Figure 1. Distribution of log-transformed *p*-values in synthetic datasets across ASHA domains.
...
Appendix Figure 2. Distribution of effect sizes in synthetic datasets across ASHA domains.
