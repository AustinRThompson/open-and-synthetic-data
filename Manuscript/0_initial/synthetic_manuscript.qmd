...
The tutorial data and accompanying code can be accessed on the Open Science Framework (https://osf.io/yhkqf/). To get started, download R (https://cran.r-project.org/) and an interface like RStudio (https://posit.co/download/rstudio-desktop/). Open the *open-and-synthetic-data.Rproj* file in RStudio. To reproduce the tutorial code only, open the file called *tutorial_script.R*. To reproduce the manuscript, open the file called *synthetic_manuscript.qmd*.
...
swallowing_data_original <-
  ...
  dplyr::mutate(bolus_consistency = as.factor(bolus_consistency),
...
base::saveRDS(object = swallowing,
  ...
  swallowing <- base::readRDS(here::here(out,"analysisData.RDS"))
}
```
...
ggsave(plot = fig1a_combined,
       filename = here("manuscript", manuscript_version, "tables-figures", "figure_1.png"),
       dpi = 300,
       bg = "white",
       width = 9,
       height = 5)
...
out <- here::here("Manuscript", manuscript_version, "result_data","02_Articulation")
dir.create(out, showWarnings = F)
# Import original data ----
...
ggsave(plot = fig2a_combined,
       filename = here("manuscript", manuscript_version, "tables-figures", "figure_2.png"),
       dpi = 300,
       bg = "white",
       width = 9,
       height = 5)
...
out <- here::here("Manuscript", manuscript_version, "result_data","03_Fluency")
dir.create(out, showWarnings = F)
...
```{r voice, echo = FALSE}
out <- here::here("Manuscript", manuscript_version, "result_data","04_Voice_Resonance")
dir.create(out, showWarnings = F)
...
out <- here::here("Manuscript", manuscript_version, "result_data","05_Hearing")
dir.create(out, showWarnings = F)
...
out <- here::here("Manuscript", manuscript_version, "result_data","06_AAC")
dir.create(out, showWarnings = F)
...
base::saveRDS(object = cognition,
  ...
  cognition <- base::readRDS(here::here(out,"analysisData.RDS"))
}
```

```{r social_comm, echo = FALSE}

out <- here::here("Manuscript", manuscript_version, "result_data","09_Social_Communication")
dir.create(out, showWarnings = F)
...
ggsave(
  filename = here::here(
    "manuscript", manuscript_version, "tables-figures","figure_3.png"),
    pval_combined,
    height = 10,
    width = 10,
    dpi = 300,
...
# save figure
ggsave(
  filename = here::here("Manuscript", manuscript_version, "tables-figures","figure_4.png"),
    es_combined,
    height = 10,
    width = 10,
    dpi = 300,
...
doc <- read_docx(here("manuscript", "templates-data", "template.docx"))
doc <- body_add_flextable(doc, value = t2)
fileout <- here("manuscript", manuscript_version, "tables-figures", "table4.docx") # uncomment to write in your working directory
print(doc, target = fileout)
```
